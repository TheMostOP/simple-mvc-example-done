...
// Get dog name will return the name of the last added dog.
...
// Function to create a new dog in the database. Works like the one for cats.
...
// It works like the cat one, except it also adds one to the dogs age.
// This is because dogs are introverted in the lore of class 430 (everyone's favorite class)
// So the horror of being percieved ages the dog by an entire year.
const searchDogName = async (req, res) => {
  ...
  // If we do not find something that matches our search, doc will be empty.
  if (!doc) {
    return res.status(404).json({ error: 'No dogs found with that name' });
  }
